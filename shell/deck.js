...
(function(){
  ...
  /* How many columns ONE SHEET uses, and whether it is too dense to carry descriptions. Pure
     ...
     Its argument is a SHEET's entry count, not the deck's: since T-036 the cap below keeps that at
     16 or under, so the dense band and the hard limit past it are unreachable through the shipped
     rule. They are kept, not dead: they are what a sheet would do if the cap ever moved, and
     DS-226's two numbers would otherwise have no instrument left measuring them.
  ...
  /* The sheets, as arrays of manifest entries.
     ...
     what stops 17 slides printing sixteen boxes and then one - a lone box on a sheet is the
  ...
  function contentsSheets(man){
    ...
    var runs = splitLongRuns(stageRuns(man), CONTENTS_CAP);
    var k = sheetsNeeded(runs, CONTENTS_CAP);
    var lo = 1, hi = CONTENTS_CAP;
