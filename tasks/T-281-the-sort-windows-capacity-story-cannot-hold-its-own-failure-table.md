...
| The rebuilt slides **looked at** | pass | **The owner looked, 2026-08-30**: the labels read clear and the headroom is legible. The lane's geometry had been recomputed from the figure's own scale without a render, which is exactly what rule 6 exists for — and it held. [`docs/OWED-LOOKS.md`](../docs/OWED-LOOKS.md) row 9 |
...
| 2026-08-30 | (no change) | **The owed look came back the same day and it passed.** The bar was resized by arithmetic on the figure's own scale rather than by rendering it, so the risk was label collision on a 28% shorter lane. The owner confirms the three labels read clear and that the 43 minutes of headroom before the second trunk is legible. |
