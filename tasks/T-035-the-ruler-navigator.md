...
- **A second tick style ships beside the first, as a theme parameter rather than a fork.**
  ...
  moved onto *unnamed*, and the twelve dots T-028 removed were unlabelled. **`dot` became the
  shipped default on the owner's ruling the same day**, with `bar` kept as the alternative — the
  point of building it as a parameter is that the choice cost one attribute rather than a rewrite.
  The degraded marks needed restating square for it: they are bar-shaped, and `border-radius:50%`
  on unequal sides is an oval. — 2026-08-08
...
| Minimum mark size measured at the 0.5 floor on a 1× display, not chosen | **not met, and closed as such** | Measured — **4 du = 2.02 CSS px** — and the binding constraint turned out to be the *inactive* marks rather than the current one, which the numbers alone would not have shown. But the criterion asks for a judgement **on a 1× display**, and it was taken through a screenshot, which is a poor instrument for one to two pixels (**L-15**). The number ships **with its doubt stated in DS-217** rather than as a settled floor. It binds only past the mode change at 17 slides, so nothing shipping depends on it; the first deck that reaches the degraded mode confirms 4 or raises it. Closing on an unmet criterion with a reason, rather than rewording it to match what was done |
...
| The deck is **looked at** at 1920 and at the 0.5 scale floor | **met** | Looked at in both styles, at both scales, and in the degraded mode. **The owner reviewed the ruler on 2026-08-08 and ruled the dot style the default** — *"exactly my expected result"* — which is the verdict this row exists for |
...
| :--- | :--- | :--- |
| 2026-08-08 | → done | **Closed on the owner's review, with one criterion honestly unmet.** The ribbon is gone; the indicator is a ruler at **624 du against the ribbon's 1180**, and its width no longer depends on what the stages are called — which was the whole problem, the row having been measured **at capacity** rather than one stage short of it. The owner reviewed both tick styles and ruled **dot** the default; `bar` stays as the alternative, and the switch cost one attribute, which is the return on building the style as a theme parameter (**rule 4**) rather than choosing a shape. **The mark floor closes as `not met`**: 4 du is measured and the *inactive* marks turned out to be the binding constraint, but the criterion asks for a 1× display and the reading came through a screenshot. The number ships **with its doubt written into DS-217** — it binds only past the mode change at 17 slides, so nothing shipping depends on it. Rewording the criterion to match what was done would have been the dishonest close. **Both defects found this session were in the instrument, not the deck**, and both looked like CSS bugs: `*` missing pseudo-elements, and pinning motion without restoring the risen state. Both are now fixed in `render.py` and written into **L-26**, which gained the rule that pinning is two obligations and half of it is worse than none. Eight gates green. [T-016](T-016-the-interaction-and-motion-layer.md) inherits the on-demand slide index and the `Escape` conflict this task recorded for it. |
| 2026-08-08 | (no change) | **The dot style became the default on the owner's ruling, and the change cost one attribute.** `data-ticks="dot"` — big dots for stage starts, small for sub-pages, accent plus a ring on the selection, the ring sliding between positions with ease-in-out. That it was a one-attribute switch rather than a rewrite is the whole return on having built the tick style as a **theme parameter** (CLAUDE.md rule 4) instead of choosing one shape and committing to it. `bar` stays as the alternative. One thing had to follow: the degraded marks are bar-shaped, and `border-radius:50%` on unequal sides is an oval, so the dot style restates them square at the measured **4 du** floor. **Two capture defects were found on the way, both in the instrument rather than the deck, and both looked like CSS bugs.** `*` does not match pseudo-elements, so [`render.py`](../tools/deck/render.py)'s motion-pinning covered every element and none of the `::before` marks — captures photographed transitions mid-flight, which is what made the dots render as ovals through three rounds of chasing specificity that was never wrong. And pinning `animation:none` **without** restoring `.rise` froze every risen element at opacity 0 and photographed a blank slide — DS-224's mechanism exactly, and the reason `render.py` has always paired the two. Eight gates green with the new default. |
