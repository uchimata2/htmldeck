...
| :--- | :--- | :--- |
| 2026-08-11 | (no change) | Shipped in **`v0.2.0`**, the release this task and [T-086](T-086-check-that-every-ledger-row-appears-on-the-slides-its-used-on-names.md) close. Its answer also corrected [`PUBLISHING.md`](../docs/PUBLISHING.md) §8, which still described the `spec.py` line as running on one deck *until this task answered*: the answer is in, and that line runs on `sort-window` alone permanently. |
