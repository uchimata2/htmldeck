...
| Printed from a **double-clicked** file and looked at (**L-01**, **L-35**) | **met** | Five decks. Four printed through Chrome's own print path offline and read out of the PDFs' own rectangles, which is the only instrument that sees this page's fault class (**L-76**) — and then **the owner opened `examples/reference-deck.html` and printed it from the dialog**, 2026-08-13. **The two prints agree to a tenth of a point**: 14 pages on a 1440 × 810 pt box, 13 cards in rows of 4/4/4/1, gaps `+19.5 +20.2 +19.5`, last card 712.1, footnote 735.7–751.5, eyebrow reading `CONTENTS` with no marker. That is worth more than the tick — it is the first time the headless path and the dialog path have been compared on this page, and **L-35** distrusts the first for reasons this measurement does not overturn but does bound: on this deck, on this page, they are the same rendering |
...
| :--- | :--- | :--- |
| 2026-08-13 | (no change) | **The last criterion closed, and it returned more than a tick.** The owner opened the deck and printed it from the dialog. That print and this session's headless one agree to a tenth of a point on every number the contents page has — page count, page box, card rows, all three row gaps, the last card's foot and the footnote's band. **L-35** distrusts a headless print and the reasons for that stand; what this bounds is where: on this deck, on this page, the two paths produce the same rendering. Recorded here rather than promoted to a lesson, because one page is not a general result. |
