...
- none. Cancelled at `specify`, so this section was never entered. What survived the cancellation is the 2026-08-07 log row, not a decision taken here.
...
| 2026-08-23 | (no change) | **§3's two template slots filled, and nothing about the cancellation changes.** taskmd `0.6.0`'s `ABANDONED SLOT` check found `- <decision — rationale — date>` still standing in a `cancelled` record, where nothing was ever going to fill it. Both slots now read *none*, with the reason. What was decided here is still the 2026-08-07 row above. Raised and closed by [T-221](T-221-answer-the-three-defects-taskmd-0-6-0s-wider-check-set-found.md). |
