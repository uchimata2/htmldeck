...
| 2026-08-18 | review → done | **Shipped in `0.4.0`.** The closing bar's step 3 is what held this at `review`: the menu had never been drawn open. Doing it found the nested frame, fixed here; doing the dark row at 25 found [T-178](T-178-dense-mode-drops-the-position-mark-below-the-section-marks.md), filed rather than fixed because both rules behind it predate this task. The generalisable half is [L-117](../docs/lessons/L-117.md) — a shut disclosure is `display:none`, so the gate suite's coverage of how it looks open is exactly zero and reads as full. `0.4.0` is a **minor** under the rule this release had to write into [`PUBLISHING.md`](../docs/PUBLISHING.md) §8, because §8 had none and the precedent contradicts itself in both directions. |
