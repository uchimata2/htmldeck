...
- The probe is left in place, rewritten with `paths:`, carrying a marker and no rule content. [T-295](T-295-complete-t-288s-observation-and-decide-the-move.md) is what deletes it.
- **It was meant to be untracked and it was not.** This entry read *it is untracked and reaches no clone* until the batch's landing, and by then `git add -A` had committed and pushed it — a test instrument published in a plugin repository, in the same commit that argued a clone should not receive machine-shaped instruction. It is untracked now and named in `.gitignore`; the pushed commit stays, because `../CLAUDE.md`'s rule about history is *before the first push, never after*, and two lines of marker text do not earn a rewrite. **`.claude/rules/` as a directory is deliberately not ignored** — whether this repository tracks a rule file is `T-295`'s decision, and ignoring it would take that decision quietly, which is exactly what `git add -A` just did — 2026-09-02
