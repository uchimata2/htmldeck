...
| 2026-08-18 | (no change) | **Shipped in `0.3.1`**, and it is the **only** task in that release requiring anything of an existing deck. [`PUBLISHING.md`](../docs/PUBLISHING.md) §8.1's row names what newly fails — three shell regions, `.qv-file`, `data-file` on every `.sources-open`, and the wrong-kind glyph on any multi-source mark — and the two commands plus the two hand edits that settle it. The release note carries that row so an adopter reads a new requirement rather than a regression. |
