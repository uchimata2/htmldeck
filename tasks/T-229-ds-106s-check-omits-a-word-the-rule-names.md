...
- **A look is owed, and it is small.** One SVG label changed from two lines to one — `What` / `actually sold` became `What sold` at the midpoint of the two — on `measure-first` slide 7's cycle diagram. **Look at:** that node's label sitting centred in its box, against the three sibling nodes. Every measurement is green, including `figgrid`, `markhits` and `spec.py`, and nothing here can decide whether the single line reads as well as the pair. Queued as row 1 of [`docs/OWED-LOOKS.md`](../docs/OWED-LOOKS.md).
