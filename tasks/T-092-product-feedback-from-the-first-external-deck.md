...
| 2026-08-11 | (specify) | **Kept at `PH3`, and against the size rule** — `s` would put it in PH2 on effort alone. It follows [T-089](T-089-a-withdrawn-task-was-deleted-rather-than-cancelled.md), placed the same way a day earlier: PH2 has shipped, and reopening a shipped phase is reserved for adopter defects. This report's own first line is that none of the six is a defect, which is exactly what disqualifies it. Its two companions, [T-090](T-090-spec5-cannot-parse-a-descriptive-slide-label.md) and [T-091](T-091-build-md-documents-icons-set-as-a-single-pair.md), *are* defects and went to `PH1` — the three arrived together and split across two phases, which is the point of filing them apart. |
