...
| :--- | :---: | :--- |
| *n* + 1 pages, no blank page at either end | **met** | **13 pages confirmed by the owner on paper, 2026-08-08** — 12 slides + 1 contents page, no blank sheet at either end. The mechanism behind it was checked at runtime rather than by reading the selector: with the contents section inserted first, `section.slide:last-of-type` still matches slide 12 (`matchesRule: true`), which is what keeps the trailing blank page away. Both the count and the mechanism now have evidence, and they agree |
...
| :--- | :--- | :--- |
| 2026-08-08 | → done | **Closed on printed evidence: the owner confirmed 13 pages, and both the qualitative and quantitative checks passed.** That settles the last two criteria together — the page count, which is the failure this deck has already paid for twice and which no script here can see, and the print-and-look itself. Every criterion now has a verdict backed by something other than the stylesheet. **One lesson was promoted rather than left in this file: [L-38](../docs/LESSONS.md) — a sweep is blind in the direction its brief did not name.** This task's spec said the bound must be measured *on a deck longer than twelve slides*, so the instrument only ever ran upward, returned a correct bound of 16, and stayed silent about seven — where the page was worse than at any measured count. Its second half is the `0.19 lines` written up as *"description gone"*: the number was right and the word for it was not, and everything downstream reasoned from the word. Both belong beyond this task, which is why they are in `LESSONS.md` and not only here. [T-036](T-036-the-second-contents-page-for-long-decks.md) carries the one thing deliberately not built. |
