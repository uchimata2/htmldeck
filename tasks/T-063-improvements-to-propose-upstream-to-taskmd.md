...
| :--- | :--- | :--- |
| 2026-08-12 | (no change) | **A sixth item was considered and refused, and it is recorded here because this is where the channel's precedent lives.** taskmd 0.5.0's `DUPLICATE INDEX` advisory fires on `docs/BRIEF.md` and always will; the obvious proposal is a per-document opt-out in taskmd's own `<!-- taskmd: -->` vocabulary. [T-098](T-098-check-reports-briefs-phase-tables-as-a-second-index.md) refused it for what it is rather than for its size — a silencer for an advisory whose value is that it cannot be silenced — and accepted the line locally instead, pinned to the file that earns it. Worth knowing before the next *should this go upstream* question: two of this project's five items came back changed by measurement, and the discipline that produced that is the same one that stops this one being sent. |
