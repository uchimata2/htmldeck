...
| :--- | :--- | :--- |
| 2026-08-09 | (no change) | **Both failures settled by [T-052](T-052-two-hard-judge-failures-in-the-reference-deck.md), and the checklist is 24 rules rather than 25.** DS-036 was amended — its *never load-bearing* clause now binds the 16–17 marginalia band its own second sentence names, rather than the whole 16–18 range, because the wide reading collided with DS-026's requirement that the deck show a visible legend. DS-208 went the other way: the deck's headline changed. And **DS-107's `Check` moved from `judge` to `—`**, which §4 above recorded as a ruleset observation and left alone — so the hard-judge jurisdiction is now **24**, and the 25 stated throughout this file is the figure as it stood when the checklist was built. |
