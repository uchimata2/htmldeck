...
| :--- | :--- | :--- |
| 2026-08-18 | (no change) | **Shipped in `0.3.1`.** Every rule change here reaches an adopter as a *loosening* — DS-138 narrowed, DS-041 split so the gate keeps only the half it can decide — so [`PUBLISHING.md`](../docs/PUBLISHING.md) §8.1's row for that version records this task as requiring nothing of a deck, rather than leaving it inferred. **DS-138 was extended once more the next day** by [T-114](T-114-the-chrome-row-layout-the-pager-deserves-the-corner.md), which tested the new boundary before spending it and found the narrowing had released the multi-source mark as well — the one thing the exemption's own test said it must not do. |
