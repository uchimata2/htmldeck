...
| :--- | :--- | :--- |
| 2026-08-11 | (shipped) | **Shipped in `0.2.1`.** DS-005 judges the argument from this release on, so a deck inlining an ESM library the way R6 measured can pass the gate. |
...
| 2026-08-11 | → proposed | Created from [T-019](T-019-build-the-capability-preflight-the-deck-ships-wit.md) §3, which found it by writing a preflight row that probes `import()` and noticing the row could never ship. `PH3` by [`../CLAUDE.md`](../CLAUDE.md)'s rule: it is not a defect an adopter hit in the published plugin — no deck can reach it — and PH2 has shipped, so everything that is not such a defect goes to PH3 whatever its size. |
