...
- **Is the iteration cap 2 or 3?** R1's pipeline has build → review → owner review → fix, which is
  effectively two machine iterations before a human sees it. — this task, against a real deck
...
| **The rubric finds seeded known defects** | **NOT MET** | **No deck exists to seed.** This is the one criterion that cannot be met until a real 12-slide deck is built, and it is the criterion that decides whether the numbers mean anything. See below. |
