...
| :--- | :---: | :--- |
| Every rule row carries an unambiguous, machine-readable answer, readable by a program that has never heard of any individual rule | **met** | **158 of 158** rows, `0 not yet`. The reader in §3 keys on nothing but the table shape and was **made to fail first** (**L-04**): a value outside the vocabulary, a dropped cell and a reason-stripped `off-gate` were seeded into a copy and all three were caught |
| A rule marked unreachable carries its reason **in the ruleset**, not a cross-reference to a tool's output | **met** | Four rules are not `yes` — DS-042, DS-072, DS-210, DS-211 — and the reader reports `rows marked never/off-gate with no reason: 0`. Each reason is in the rule's own cell |
| The four reasons `audit.py` prints (DS-033, DS-061, DS-065, DS-072) are in the ruleset, and `audit.py` no longer holds the only copy of any of them | **not met** | **The criterion rested on a premise `implement` disproved.** Only **DS-072**'s note is a reachability reason and only it moved. DS-061 and DS-065 say the rule *is* checked — statically, from source, rather than at render — and DS-033 appears only inside DS-065's note. Those three are gate-internal facts about *which stage* checks a rule, and they are `yes`, so `audit.py` **does** still hold the only copy of three of the four. Read as written: not met → [T-039](T-039-finish-the-record-t-037-left-in-the-wrong-places.md) |
| §11's "conditions 22 and 30" are written off as unrecoverable **in the ruleset**, with the reason | **not met** | The write-off exists and is correct, **in the wrong document**: [`DESIGN-RATIONALE.md`](../docs/DESIGN-RATIONALE.md) §5.5. `DESIGN-SYSTEM.md` opens *"The operative ruleset. Nothing else"*, so the rationale is not the ruleset and the criterion says the ruleset → [T-039](T-039-finish-the-record-t-037-left-in-the-wrong-places.md), which also carries the question of whether the criterion asked for the right place |
| **Every** surviving `§11` reference in `tasks/` is corrected | **not met** | The criterion named T-004, T-005 and T-030 and those three are annotated — but the binding word is *every*, and **three more were missed**: [T-014](T-014-synthesise-research-into-the-design-system-reference.md):248 *"§11 gained seven conditions"*, [T-021](T-021-the-reflow-view-and-the-resolution-contract.md):298, which explains the staleness as *"a numbering T-022 removed"* — the wrong cause, since nothing was removed — and [T-022](T-022-split-the-design-system-from-its-rationale.md):107 *"six stage/reflow conditions were prose in §11"* → [T-039](T-039-finish-the-record-t-037-left-in-the-wrong-places.md) |
| A program can compute *"rules the gate is expected to cover"* from the ruleset alone, and the number is stated for T-005 | **met** | **105** — `Reach: yes` and `Check` in {`auto`, `render`} — computed from the two columns and stored nowhere. Cross-checked against the `Check` totals: `auto` 62+1+2 = 65, `render` 43+1 = 44 |
| `DESIGN-RATIONALE.md` records why the distinction is carried per rule rather than in a list | **met** | §5.5, which states the general form: a parallel structure can go absent without anything breaking, because everything else works off the originals |
| No rule's `hard` / `default` / `guidance` label changes as a side effect | **met** | Diffed the Label cell of every rule between `db24d39` and `HEAD`: **158 rules before and after, 0 labels changed** |

**Five met, three carried.** All three failures are the same shape and none is in the deliverable
itself: the column, its vocabulary, its reader and its number are all as specified. What is wrong is
**where three pieces of the record were written** — one in the rationale instead of the ruleset, one
scoped to three files when it said every, and one written against a premise that turned out false.
That is worth noting because it is the same class of defect this task exists to correct, appearing
in the task that corrects it.

**Child fix tasks raised**
- [T-039](T-039-finish-the-record-t-037-left-in-the-wrong-places.md) — carries all three.

## Log

| Date | Status change | Note |
| :--- | :--- | :--- |
| 2026-08-09 | → done | **Five of eight criteria met, three carried to [T-039](T-039-finish-the-record-t-037-left-in-the-wrong-places.md).** The deliverable is sound and that is what the split says: the `Reach` column is on **158 of 158** rule rows, its reader keys on nothing but the table shape and was **made to fail first** on three seeded defects (**L-04**), **105** is derived from the ruleset and stored nowhere, and **0 of 158 labels changed** — checked by diffing the Label cell between `db24d39` and `HEAD` rather than by trusting the edit. **All three failures are about where a piece of the record went, not about what was built**, which is the same defect class this task exists to correct, occurring inside it — worth writing down rather than smoothing. *(1)* The conditions 22/30 write-off is in `DESIGN-RATIONALE.md` §5.5 and the criterion said *the ruleset*, which `DESIGN-SYSTEM.md` declares itself to be and the rationale is not. *(2)* The `§11` sweep covered the three files the criterion **named** and the criterion said **every** — T-014:248, T-021:298 and T-022:107 were missed, and T-021's is the one that misleads, recording the cause as *"a numbering T-022 removed"* when nothing was removed. *(3)* The criterion about `audit.py`'s four notes was written believing all four were reachability reasons; `implement` disproved that for three, so it is **not met as written** rather than quietly reinterpreted — the honest verdict, since rewording it to match the outcome is what [`TASK-WORKFLOW.md`](TASK-WORKFLOW.md) §2 calls a description rather than a criterion. |
