...
2a. **The good failure looks like a broken tool, and reading it that way costs the lesson.** B22
   wrote a fixture for `run_probes.harvest_title_channel` on 2026-09-02 and it died on
   `chunks[0]` — which reads as the off-by-one the function's own comment warns about. It was
   the **fixture's** model that was wrong: `build_probes.py` emits `k % chunks.length`, so the
   index is zero-based against a one-based total, and the tool had been right for a year. Derive
   the model from the producer before believing the failure, or a correct tool gets “fixed”.
