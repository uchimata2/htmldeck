...
**From the htmldeck project, one machine, two shells.** Four observations, none ranked and none
a request. *`O-C3` was added 2026-08-20 and `O-C4` on 2026-08-22, after the first two had been
recorded; the disposition note above is unchanged and still covers the document.* This document exists because the first of them was originally filed against the wrong
...
| **O-C4** | **A heredoc written through the shell tool loses one level of backslash, and a long one can fail to parse at all.** Two modes, measured on this machine. The first is silent: text passed through `<<'BLOCK'` arrives with one backslash removed even with the delimiter quoted and including `cat > file`, so a doubled escape lands as a real control character - a `\n` inside a generated Python string literal becomes an actual newline and the file stops parsing. A single backslash survives, so the effect is uniform rather than random. The second is loud but misattributed: measured 2026-08-22, two heredoc bodies of a few hundred lines each, carrying mixed `'` and `"`, returned `unexpected EOF while looking for matching '` naming a line **inside the body**, and the target file was never created - while a dozen shorter bodies in the same session went through untouched. **Nothing distinguishes either from a mistake in the text being written**, which is why the first cost this repository four corrupted edits to one file in a single session and a `git checkout` to recover. Recorded here for `O-C1`'s and `O-C3`'s shape: a value correct where it is written, altered where it is used, and nothing in between saying so. **The direction worth considering is the same as `O-C1`'s second** - a body that cannot be passed through whole is worth an error naming the transformation, rather than a shell parse error pointing at the author's own text |
