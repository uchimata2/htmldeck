...
| DS-244 | **No diagram label is set on top of another diagram label.** Two labels whose boxes share more than **30%** of the smaller one's extent *in both axes* are a collision: at that depth the glyphs interfere and the reader resolves them one at a time, or not at all. The threshold is a fraction rather than a design-unit figure on purpose - an absolute number states one face at one size and has to be re-tuned for every deck that sets its labels larger, which is how a tolerance stops meaning anything. Measured on the rendered page, in the slide's own coordinates, by `tools/deck/markhits.py`, which `check.py` reads. **A label set on a LINE is deliberately not this rule.** The same tool measures it and reports it and never fails a deck for it, because a label placed along the line it names is ordinary chart vocabulary and is geometrically indistinguishable from a label a line ran over: counted across the four shipped decks it fires 16 times for one real defect, and the obvious discriminator inverts - the real defect grazes the label's edge while the deliberate placements run through its centre. *Added 2026-08-21 by [T-204](../tasks/T-204-an-instrument-for-mark-collisions.md), after the portfolio-review deck passed every gate here while carrying fifteen chart defects that only people found, and three separate looks each missed some. Calibrated per [T-115](../tasks/T-115-the-specification-can-assert-a-layout-the-shell-cannot-honour.md)'s rule: the half that gates is 1 true hit and 0 false alarms over 30 slides, and the half that would have been 15 false alarms reports instead.* | hard | render | yes |
...
| ID | Rule | Label | Check | Reach |
| :--- | :--- | :--- | :--- | :--- |
| DS-140 | **A suggested starter set of motions, and an admission test that does not depend on a name.** The set — **Rise** (entry, staggered) · **Current** (flow, dashed, looping) · **Open/Turn/Scale** (reveals) · **Pulse-once** (never looping) — is what the shipping theme carries and what a deck gets without designing anything. **It is a starting point, not a boundary.** A motion outside it is admissible when it passes the test, and the test is the rest of §5.2 read as a whole rather than a list kept here: it **encodes something** (DS-150), it **does not drive the page's design** (DS-243), it **declares its kind** (DS-237), it sits **inside DS-141's band or declares a licence to leave it**, and it survives **reduced motion** (DS-143), **print** (DS-224) and **the stop control** (DS-218). A motion that passes is conformant whether or not it has a name here; a motion that fails is a defect however familiar its name. **Entries and reveals sit inside DS-141's 500 ms cap; Current runs 3–6 s and Pulse-once 0.8–1.6 s.** The shipping theme's values are 340 ms at `cubic-bezier(.22,1,.36,1)` with a 60 ms stagger, dasharray 7 6 over 4.5 s, 380 / 420 / 300 ms, and 1.2 s. *Amended 2026-08-09 by [T-007](../tasks/T-007-define-the-parametric-theme-layer.md): the durations **were** the rule, and the check admitted a long motion only at exactly 1.2 s or 4.5 s, so a theme that changed motion failed the gate for being a second theme. **The names and the closure are what this rule is for** — [`DESIGN-RATIONALE.md`](DESIGN-RATIONALE.md) §4 argues that a named vocabulary is what stops animation becoming decoration, and argues nothing about the milliseconds. So the vocabulary stays fixed at four and the numbers become bands, with the shipping theme's inside them.* **Opened 2026-08-21 by [T-187](../tasks/T-187-open-the-motion-vocabulary-into-a-style-guide.md), on the owner's ruling that any animation aligned with the rules is admissible and the four names are a suggestion.** *The closure is what changed, and the reason it could go is that §4's argument has a better answer than a list. A name never tested anything: it stood in for* this motion was considered *, and the four were the corpus's four rather than a derivation. **DS-243 states what the closure was standing in for** — the page is not designed around its animation — so the guard that was carried by an allow-list is now carried by the rule it was always a proxy for, and it reaches a motion nobody has named yet, which a list by construction cannot. What did **not** change is every rule protecting observable behaviour: DS-141's cap, DS-143, DS-218 and DS-224 are `hard` and unchanged in force, and DS-141's exception stopped being a name (see its row). The written guidance the owner asked for is [`MOTION-GUIDE.md`](MOTION-GUIDE.md), which is guidance and gates nothing.* | hard | auto | yes |
| DS-141 | **Entry and transition animations max 500 ms**, and **eased rather than linear**. Inter-slide transition 400–500 ms. **DS-141 governs entry and transition only, and the licence to exceed the cap is declared rather than named.** A rule that starts a motion running longer than 500 ms declares **`--motion-long`** on that rule, and its value says why: **`loop`** — the motion repeats, and DS-218's stop control follows from it; **`illustration`** — the motion *is* the content, because the slide's subject is the process it animates; **`emphasis`** — a once-only mark held long enough to be seen; **`request`** — the deck's author asked for it. **A duration over 500 ms in a rule that declares no licence is a defect**, and that is the whole of what a gate decides here. **The reason values are a closed set and the motions are not**, which is the distinction this rule now rests on: they constrain *why the cap may be exceeded* — the one axis carrying the safety property — and say nothing about what motions exist, so any motion, named in DS-140 or not, may carry any of them. *Re-derived 2026-08-21 by [T-187](../tasks/T-187-open-the-motion-vocabulary-into-a-style-guide.md). Until then the waiver was `is one of DS-140's four`, which the gate implemented as* the declaration reads `--pulse-dur` or `--current-dur` *— so opening DS-140 would have left the cap with nothing to test and dropped it out of the `auto` column, which is the coverage this project counts. Declaring the licence keeps it `auto` and buys something the names never gave: the reason is on the artifact, so the critique pass can argue that a motion claiming `illustration` is decoration, where before it could only observe that the motion was called Pulse.* *Amended 2026-08-09 by [T-016](../tasks/T-016-the-interaction-and-motion-layer.md): the rule said **ease-in-out**, which is one theme's curve stated as the ruleset's — the same defect T-007 found in DS-034's line height and DS-140's durations, one axis along. What the 500 ms cap and its rationale argue is that a transition is short and eased; nothing argues that curve. So **every named motion carries an easing token** and a theme chooses the feel, which is what makes a deliberate overshoot on a reveal a design decision rather than a gate failure. `linear` survives where the mechanism requires it — a looping dash, a zero-duration step — and nowhere else.* | hard | auto | yes |
...
| DS-146 | Charts draw in **once**; never re-animate on back-navigation. **The draw-in is Rise applied to the chart's marks, staggered** — the same entrance the rest of the slide uses, so the chart arrives as part of the slide. **A stroke-dash draw is the specific thing this rule refuses**, and the reason is DS-243 rather than a count: a line drawing itself makes the reader watch the drawing instead of read the finished shape, so the chart's timing becomes the slide's event and the figure is designed around its own animation. The dash is also a **looping** mechanism — it is how `Current` says *this is a flow* — and spending it on a once-only arrival tells the reader the chart is moving when it has arrived. *Re-derived 2026-08-21 by [T-187](../tasks/T-187-open-the-motion-vocabulary-into-a-style-guide.md), which opened DS-140. The rule read "would add one to a vocabulary DS-140 fixes at four", so it reasoned entirely from the closure and would have lost its whole justification with it — while the thing it forbids is still wrong for reasons that were never about counting.* | hard | render | yes |
...
| DS-243 | **The page is not designed around its own animation.** The slide's structure, its figure and its pacing are chosen by the argument; the motion is applied to what the argument already wanted. The failure this names is the reverse order — a slide built as a flow because the author wanted a flow to animate, a figure split into six parts because six is a good stagger, a number promoted to a headline because it was the one thing left worth counting up. **Three exemptions, and they are the deck's subject rather than the author's taste:** the slide is *about* motion, transition or process; the author asked for significant motion, having been told what it costs; or the deck's theme is an animation-rich or 3D style, which is a decision taken once for the deck and not per slide. **This is not DS-150 said twice.** DS-150 asks whether a motion encodes anything and rejects the motion when it does not; this rule asks what the page was shaped by, and it fails a page whose motion encodes something real — the flow *is* a sequence — where the argument never called for a flow. A deck can pass DS-150 on every animation it carries and fail this on the slide those animations were invented for. *Added 2026-08-21 by [T-187](../tasks/T-187-open-the-motion-vocabulary-into-a-style-guide.md), from the owner's sixth motion principle, which is the one principle of the six that no rule stated — 3 and 4 already restate DS-141 as T-016 amended it. **It is a rule and not guidance because it is what DS-140's closure was standing in for.** [`DESIGN-RATIONALE.md`](DESIGN-RATIONALE.md) §4 rests the project's whole motion position on the claim that a named vocabulary is what stops animation becoming decoration; open the vocabulary and that argument is left with no answer, and the critique pass with nothing to argue from. Guidance would not have carried it: `MOTION-GUIDE.md` is where the other five principles went, and the difference between the two homes is whether a deck can be failed for it.* | hard | judge | — |
| DS-218 | **Motion that loops, or runs over 5 s, ships with a persistent, keyboard-operable control that stops it** — and the deck still reads with motion off. **The trigger is any looping motion, which is what this rule has always said** — the shipped theme's `Current` is infinite and is the instance every deck meets, so **every deck with a flow diagram needs this control**, but a looping motion nobody has named yet triggers it identically. *That sentence read "DS-140's `Current` is infinite, so", which sourced the obligation to the closed vocabulary rather than to looping; [T-187](../tasks/T-187-open-the-motion-vocabulary-into-a-style-guide.md) restated it on 2026-08-21 without changing its force.* §7's 2.2.2 stated the obligation without any rule here requiring the deck to build one. Distinct from DS-143: `prefers-reduced-motion` is what the reader's system asks for; this is what the reader can reach. **`persistent` is now decided rather than read as a word.** *Extended 2026-08-18 by [T-114](../tasks/T-114-the-chrome-row-layout-the-pager-deserves-the-corner.md), which put a `More` menu on the chrome row and so created the first way to satisfy this rule in the letter and break it in fact: a stop control one click inside a shut menu is not reachable while the motion runs.* **A looping deck's control must not sit inside `.more-menu`** — `audit.py` reads the placement off the built markup rather than inferring it, which is why [`COMPONENT-CONTRACT.md`](COMPONENT-CONTRACT.md) §3.4 makes the position a build-time fact and the `CHROME_TAIL` slot carries it. Existence had been the whole test until then, and existence decided nothing: the shell builds the control into every deck. | hard | auto | yes |
...
| DS-230 | **Tier two answers a question the face provokes about something the face already carries, and it is one of four kinds, closed:** **`derivation`** — how a figure on the face was produced; **`scope`** — what a term or figure on the face includes and excludes; **`condition`** — what the claim needs in order to hold, and where it fails; **`instances`** — the named members of a total the face states as a count. **Every other rule in §5.3 is a test to reject.** DS-162 says what must be tier one, DS-161 says the slide must stand closed, DS-170 says long text goes behind rather than on — and none of them says what tier two is *for*, so a deck can satisfy every one of them and still put an appendix behind the click. **Content whose subject the face does not carry is a slide, not a panel**, and that is the failure this rule names: an appendix, a continuation, or the next argument arriving early. **Every `.disc` declares its kind in `data-disc`** ([`COMPONENT-CONTRACT.md`](COMPONENT-CONTRACT.md) §3.3), which makes the editorial decision a claim the critique pass **verifies** rather than re-derives — the same reason the ruler writes down `data-scale` under DS-217, and the same caveat: the attribute is the author's claim, and this rule is `judge` because whether it is true needs someone to read the slide. **The vocabulary is closed for its own reason rather than for tidiness, and the reason is that these four are a test rather than a palette** — each names a question the face provokes, so the set is what stops *behind the click* becoming *wherever it did not fit*; a fifth kind is a ruleset change with a stated reason (DS-000), never a value invented in one deck. *This clause read "closed for DS-140's reason", borrowing an argument DS-140 no longer makes. [T-187](../tasks/T-187-open-the-motion-vocabulary-into-a-style-guide.md) decided on 2026-08-21 that **the closure survives, restated in its own terms**, and the decision turns on the two rules governing different species. **DS-140 governs an expressive medium**, where a motion nobody has named can be a legitimate design act and the owner's ruling is that any animation aligned with the rules is admissible — so a list there forbids good work it has never seen. **DS-230 governs a rejection test**: its four kinds are not four ways of saying something, they are the four questions that make a panel a panel, and a fifth kind is not a new expression but a new claim about what belongs behind a click — which is exactly the "wherever it did not fit" failure this rule exists to name. Opening DS-140 therefore does not open DS-230, and nothing here is opened by inheritance. The owner's principles are about animation and say nothing about disclosure; this is reversible on their word, and stated here rather than left as an open question so that the next reader of this row inherits a decision instead of a dangling one.* *Added 2026-08-09 by [T-016](../tasks/T-016-the-interaction-and-motion-layer.md), extracted from the reference deck's ten panels — 3 `derivation`, 3 `scope`, 3 `condition`, 1 `instances` — which conformed to it before it was written. **The label's verb does not decide the kind; the rows do:** "How the corridors are defined" opens like a derivation and is four definitional boundaries, which is `scope`.* | hard | judge | — |
...
| DS-224 | **Entrance animations are disabled for print.** They hold their pre-animation state until played, so a slide the reader never advanced to prints blank or half-risen — the deck's own motion (DS-140) turned into missing content on paper. This is DS-221's mechanism on a second medium: any rendering that captures the deck without playing it must pin motion off first. | hard | render | yes |
...
| DS-221 | **Pin motion off before capturing.** A looping motion — the shipped theme's infinite `Current` is the one every deck carries — means a headless render never reaches a quiescent state, so the screenshot fires mid-transition and yields a convincing blank slide. A render gate that does not disable animation is measuring an arbitrary frame and reporting it as the deck. | hard | — | — binds whoever builds a check, not the deck; there is nothing here for a gate to test |
