...
| `0.2.4` | 2026-08-14 | 2 | **Nothing an adopter loads** — PH3 record work only, and the version moved because the published line takes the next patch number rather than because the plugin did. [`PUBLISHING.md`](PUBLISHING.md) §8.1 carries the row, which is what makes an absent row distinguishable from a forgotten one |
...
| `0.7.0` | 2026-09-03 | 71 | **The release the repository read itself end to end.** The pre-release audit ([T-219](../tasks/T-219-pre-release-audit-of-the-whole-repository.md)) covered every tracked file across 43 cycles and raised **129 findings**, each carrying the command that proves it; all eight `High`, all 70 `Med` and all 51 `Low` are closed or accepted with a reason and a date, through 33 remedy tasks. **Eighteen more fixes came from outside** — an adopter's twenty-seven reports, kept in the adopter reports T-320 removed and worked as `T-254` to `T-271`. **The two shape changes that make this a minor**: `--accent-ink` is retired from the theme contract ([T-275](../tasks/T-275-retire-accent-ink-from-the-contract-the-themes-and-the-decks.md)) and `.turn` is a new contracted part, so DS-140's card reveal is a component rather than two dials wired by hand ([T-274](../tasks/T-274-build-the-card-reveal-so-turns-two-dials-have-a-reader.md)). **Four rules narrowed rather than tightened** — DS-092, DS-100, DS-110 and DS-236 — each having failed decks for something it was never written to prevent; the ruleset holds at 177 rows with none added, none removed and no rule's gate column moved. **Phase 2's own finding is the one worth carrying out of the run** ([L-163](lessons/L-163.md)): the audit predicted a per-finding effort 129 times and left 13 gradeable, because its own triage ruling changed the unit the work was recorded in. Cost the unit that will be worked, not the unit that is found. |
| `1.0.0` | 2026-09-15 | 30 | **The marketplace release, cut when the backlog the owner set for it was empty.** Every open task outside the 3D line closed first, by the owner's ruling of 2026-09-14, worked as wave 8's batches B25 to B29 ([`REMEDIATION-ORDER.md`](REMEDIATION-ORDER.md) §2), with [T-321](../tasks/T-321-make-a-decks-render-independent-of-its-file-length.md) exempted the next day. It goes out with the community marketplace submission [T-300](../tasks/T-300-submit-htmldeck-to-anthropics-community-plugin-marketplace.md) prepared. **Major by that ruling, where §8's own test gave at least a minor**: `.ruler-tip` became a required part ([T-307](../tasks/T-307-give-the-chrome-first-and-last-page-controls-and-a-ruler-whose-marks-a-reader-can-aim-at.md)) and the chart-engine declaration moved into the head comment ([T-311](../tasks/T-311-keep-a-decks-chart-engine-declaration-through-a-shell-sync.md)). **The first §8.1 row measured rather than derived**: today's gate against the four decks shipped at `v0.7.0` failed `DS-229` and nothing else, and `sync` alone repaired it. **The validator the submission names passed while the plugin manifest failed it**: `claude plugin validate . --strict` reads only the marketplace manifest when one sits beside `plugin.json`, and pointed at `plugin.json` it fails on the root `CLAUDE.md`, kept there for the reason in T-300 §3. |
