...
| ~~18~~ | **CE-18** | E / F5 | The docs gate is 81% one render — seconds, not tokens — **done 2026-09-14 by `T-296`, and the guarantee the ruling traded away was kept.** `check_all.py --docs` went from 25.6 s to 8.5 s on one machine, and the full run stayed at 203 s. Under `--docs`, `figures.py` compares the README's `check.py` block with itself instead of re-running it, but only while the block is identical at `origin/master`. So a commit that edits that block still pays for the render | *time* | `s` | stated | §6.3 |
...
| ~~22~~ | **CE-22** | D / F3 | Output at five times the input rate was a quarter of the session's weighted spend, and the record forms are the mechanism — **collides with the project's record policy; reported for the owner, who ruled on 2026-09-13 that it becomes a decision. Decided 2026-09-14 by `T-301`**: a task's §3 takes a shape, a lesson a 1,500-byte limit, and register rows and handoffs no change, each written where its writer reads it | *bimodal* | — | stated | §6.3 |
