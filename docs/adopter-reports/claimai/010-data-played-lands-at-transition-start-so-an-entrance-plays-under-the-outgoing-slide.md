...
| **Status** | `closed` — closed 2026-08-29 by [T-268](../../../tasks/T-268-three-chrome-and-timing-defects-in-deck-js.md). **This report's own safer option taken**: `data-played` is untouched and `data-arrived` is a second attribute, so nothing already in the field changes behaviour. `DS-146` now says when each lands and which to gate on. **Two implementations were refused by measurement before the third passed, and the first of them is worth this report knowing.** Waiting on the *arriving* slide's own animation marks arrival at t = 0 and is `data-played` under a second name — the arriving slide animates nothing, because every transition rule in `components.css` is keyed to `[data-leaving]` and `.slide[data-current]` only sets `opacity` and `visibility`. **Arrival is the OUTGOING slide finishing**, which is exactly what this report describes. The second implementation waited on `Animation.finished` and never fired: the deck's own `animationend` handler removes `data-leaving` the moment the animation ends, which cancels the animation, and a cancelled animation rejects. It lands from that listener now. **One limit, stated:** the harness advances timers and not the frame clock, so the probe drives the leave animation to its end rather than waiting it out — the event is measured, the wall-clock delay is not. |
