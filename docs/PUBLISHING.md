...
§1 to §7 are the detail behind the humanizing rule in
[`../.claude/rules/release.md`](../.claude/rules/release.md). That paragraph is the rule; those
sections are what it means in practice — the covered-set test,
...
| # | Step | What proves it |
| :-- | :--- | :--- |
| 1 | **`python tools/check_all.py` green** — the whole set, not the routine one and never `--docs`. **It takes minutes, so run it in the background**; it prints its own elapsed time. **Stage first if the release adds a tool or a deck**: the tool partition is derived from what git tracks, so an unstaged file is invisible to the run and the *next* run is the first that can fail on it as `UNCLASSIFIED` (measured 2026-08-22, T-041) | Its own last line: `0 failure(s), 0 unclassified, 0 stale` and the seconds beside them. It replaced a list of sixteen commands on 2026-08-13 ([T-096](../tasks/T-096-one-command-that-runs-every-checker-and-says-what-it-skipped.md)) |
| 2 | **Bump the version** in `.claude-plugin/plugin.json`, `CLAUDE.md` and `README.md` — to the next number on the **published** line, per the rule below the table — then `python tools/deck/shell.py sync <deck> --write` each deck in `check_all.py`'s `DECKS`, whose generator tag reads the manifest (T-318) | Three files carry it, and every shipped deck's generator tag; a grep for the outgoing version returns nothing outside `docs/BRIEF.md`'s history |
...
| 8 | **Record the shipping version** in each closed task's `shipped_in` and log, and in the chronology home — [`RELEASE-HISTORY.md`](RELEASE-HISTORY.md), which carries one row per release — **then run `python tools/docs/chronology.py`**, which holds that row's date and count to the tag and the task records; step 1's gate ran before the row existed | The version appears in the record without anyone reconstructing it later, which is the failure this whole section exists for. *It said `BRIEF.md` until 2026-08-14; [T-145](../tasks/T-145-move-brief-mds-release-phases-to-its-own-document.md) moved the chronology out that morning and `BRIEF.md` now holds no version string at all, so the step named a file that could not satisfy it. `RELEASE-HISTORY.md` was added 2026-08-18 for the same reason one step further on: the step named one of the two homes the same move created, and `0.4.0` shipped with the chronology missing its row until the next session's sweep found it. It said **both** homes, naming [`RELEASE-PHASES.md`](RELEASE-PHASES.md) as the second, until 2026-09-02. That half had no proof of its own — the stated proof is satisfied by `RELEASE-HISTORY.md` alone, and `chronology.py` fails the run without it — so it stopped running and nothing said so, for 85 tasks. `RELEASE-PHASES.md` is a decision record rather than a chronology, and a release writes nothing into it (`PR-24`)* |
...
| `0.4.0` | **The chrome row's shape** ([T-114](../tasks/T-114-the-chrome-row-layout-the-pager-deserves-the-corner.md)). New contracted parts — `.navbox`, `.more`, `.more-menu` and the `.btn--pager` modifier — and `.controls` is gone. `.chrome` carries a **twelfth slot**, `CHROME_TAIL`, holding `More` and, in a deck whose motion loops, `Motion` beside it. DS-218 is tightened from *a control exists* to **a control not shut inside `.more-menu`**; DS-217's measured bound moves 17 → 16 because the container is drawn. One new token, `--more-menu-w`. The **shared shell** moved in all three regions, and then in `components.css` once more before the tag: a menu item is drawn as a row rather than as a button in a box, which is a look fixed after rendering the menu **open** for the first time and requires nothing of a deck beyond the `sync` this row already asks for. **[T-177](../tasks/T-177-tokens-write-carries-the-dark-value-into-the-light-band.md) requires nothing of a deck** and the row says so rather than leaving it inferred: it fixes `tokens --write` carrying a dual-band colour's dark value into the light band, which is this repository's own tooling | **`shell.py check` says `NOT A SHELL`, and that is the whole report** — the deck has no `CHROME_TAIL` anchor, so the cut stops there and the three region failures behind it are never printed. It reads as *your file is broken* rather than *your shell is one release behind*, and it is the second. Then, **after the repair below and because of it**, `audit.py` — **`DS-218`** on any deck whose motion loops or runs past 5 s, because the migration puts `Motion` in the menu and a control behind a click is not persistent. Also `theme.py` / `shell.py check` — **`TOKENS`, `--more-menu-w`** | Two commands, in order: `python tools/deck/shell.py sync <deck> --write` installs the anchor and settles the three regions, announcing `MIGRATING` when it does; `python tools/deck/shell.py tokens <deck> --write` adds `--more-menu-w`. Verified on a deck built before this change: `NOT A SHELL` → three regions synced → one token → `DS-218 pass`. *There was a third, `shell.py tail <deck> --loops --write`, required on a deck with looping motion because the migration put `Motion` in the menu and this release said a control behind a click was not persistent. **[T-277](../tasks/T-277-put-motion-back-inside-the-more-menu.md) reversed that clause on 2026-08-29 and the flag is gone**, so the step is dropped rather than corrected: the migration's own result now satisfies DS-218. The account of what `0.4.0` tightened is left standing above, because what a release did is a fact; a command an adopter would run today is not, and this one would exit with an error* |
| `0.5.0` | **Four theme tokens and one rule about copy.** `--afford-dur`, `--afford-ease`, `--press-dur` and `--press-ease` are required declarations — the band a control's answer to the hand runs in (DS-240, [T-198](../tasks/T-198-give-affordance-motion-its-own-band-faster-than-content-motion.md)). **DS-241 is new and it is about words, which is the first requirement here that an adopter cannot settle with a command**: every eyebrow must name its slide's subject rather than the position, the stage or the headline's own words. The **shared shell** moved in `components.css` and `deck.js`. `data-stage` gains a third non-index value, **`front`** — additive, and a deck using no lobby is unaffected. `render.py shots` counts slides **from one**. | `check.py` on **every deck built before this release**: `DS-013` — four tokens not declared; `DS-229` — three motion rows now naming `--afford-*`; `DS-240` — the pager's press outranked by its hover, which is a real defect every shipped deck carried; and `DS-241` on any slide whose eyebrow reads `02 · Why now`. Also `shell.py check` — `COMPONENTS` and `SCRIPT` differ. **Measured on a real outside deck before the tag**, which is where three of the four came from. | `python tools/deck/shell.py tokens <deck> --write`, then `sync <deck> --write`, settles `DS-013`, `DS-229`, `DS-240` and the shell rows. **`DS-241` is the one that needs a person**: one line of copy per slide, naming what is on it. |
| `0.5.1` | **Nothing.** No rule changed and no part was added; DS-241 required this since `0.5.0` and its **check** was wrong ([T-201](../tasks/T-201-ds-241s-check-misses-an-eyebrow-written-as-an-entity.md)). | **A deck that passed `0.5.0` on DS-241 may fail now**, and the rule did not move under it - the check could not see an eyebrow whose separator is written `&middot;`, which is how a build writes it. `0.5.0`'s gate reported `0` on a deck where all fifteen eyebrows were the defect. | `0.5.0`'s row, unchanged: one line of copy per slide, naming what is on it. |
| `0.6.0` | **Seven glitch-free conditions and a collision rule, both decided on the rendered page rather than in the markup.** `GF-2` to `GF-8` ([T-041](../tasks/T-041-implement-the-nine-glitch-free-conditions.md)) decompose `../.claude/rules/decks.md` rule 2 and are answered from one browser walk: the console clean over the load and one pass; every declared `@font-face` reporting loaded; text rendered in an embedded face rather than a fallback; no slide's stage scrolling past its own box; no box moving over the font swap; every canvas surface drawing something; every slide reached first to last without a script error. **`DS-244`** ([T-204](../tasks/T-204-an-instrument-for-mark-collisions.md)) is new and gates: two diagram labels sharing more than 30% of the smaller one's extent **in both axes** are a collision. **Two declarations a motion makes about itself**, both replacing an instrument that read a name instead of a claim: `--motion-long`, DS-141's licence to run past the 500 ms cap ([T-187](../tasks/T-187-open-the-motion-vocabulary-into-a-style-guide.md)), and `--motion-subject`, which is what DS-142 now reads instead of the class name `.current` ([T-214](../tasks/T-214-ds-142s-checker-is-an-allow-list-of-one-class-name.md)). The **shared shell** moved in `components.css` and in neither other region. **Two changes require nothing of a deck and this says so rather than leaving it inferred**: DS-122 now admits a declared chart engine ([T-202](../tasks/T-202-amend-ds-122-into-a-threshold-and-bind-its-check-on-structure.md)) through one `<meta name="htmldeck-chart-engine">`, and a deck that declares nothing is held to the hand-authored default, which is where every deck today stands; and DS-142's rebinding **removes** a failure rather than adding one, since a looping motion that was not `.current` could not pass at all before. | **`check.py` on every deck built before this release, and for the first time the failures need a browser to see.** `GF-3` and `GF-4` on a deck whose faces are linked rather than embedded — which rule 1 forbade from the start and nothing here could measure; `GF-6` on a deck whose layout moves over the font swap; `GF-8` on a deck with a script error on any slide past the first. **`DS-244` on any deck whose diagram labels overlap** — it is the instrument written after the portfolio-review deck passed every gate here while carrying fifteen chart defects that only people found. **`DS-141` on a deck's own motion running past 500 ms in a rule declaring no `--motion-long`**: the waiver used to be *the declaration reads `--pulse-dur` or `--current-dur`*, so a deck reusing either token name passed without ever saying why, and now it says why. Also `shell.py check` — `COMPONENTS differs from shell/components.css`. **`GF-7` returns NO SUBJECT on a deck that draws no canvas**, which is the gate's third state and is not a pass. | `python tools/deck/shell.py sync <deck> --write` settles the shell row and is the only part a command can settle. **The rest need the deck opened in a browser**, because six of the seven conditions are facts about what the browser did and no markup states them: `python tools/deck/check.py <deck>` runs the walk and prints `GF-2` to `GF-8` with what each measured. For `DS-244`, move the colliding label — the row names both boxes and the slide it is on. For `DS-141`, add `--motion-long:<reason>` to the rule from the closed set `loop`, `illustration`, `emphasis`, `request`; for a motion that loops, add `--motion-subject:live` beside it. |
| `0.7.0` | **A contracted token retired and a contracted part added, which is why this is a minor.** `--accent-ink` is **gone from the theme contract** ([T-275](../tasks/T-275-retire-accent-ink-from-the-contract-the-themes-and-the-decks.md)): no theme varied it and every deck carried it. **`.turn` is a new part** — `0+` inside `.slide`, with a motion row `.slide[data-arrived] .turn` reading `--turn-dur` and `--turn-ease`, so DS-140's Turn is a component rather than two dials wired by hand ([T-274](../tasks/T-274-build-the-card-reveal-so-turns-two-dials-have-a-reader.md)). Three **optional** tokens arrive with DS-141's licensed long band — `--long-dur`, `--long-ease`, `--long-delay` — because the `request` licence the rule grants had nowhere to put its number ([T-264](../tasks/T-264-ds-141s-request-licence-has-nowhere-to-put-its-number.md)). **DS-010 now scans custom properties**, which it skipped outright before, so a duration hidden in `--x:1000ms` beside a policed `animation:1000ms` no longer passes where the literal fails. The **shared shell moved in all three regions**. **Four rules narrowed rather than tightened and this says so rather than leaving it inferred**: DS-110 by where a raster sits, DS-100 to the question a slide puts in its *header*, DS-092 by excluding provenance from its paragraph half, and DS-236 to the text column a diagram sits in rather than the slide's. Each had been failing decks for doing something the rule was never written to prevent. | `shell.py check` on **every deck built before this release**: `SKELETON`, `COMPONENTS` and `SCRIPT` all differ. `check.py` — **`DS-013` on any deck that declares `--accent-ink`**, because the contract no longer names it; **`DS-010`, and `DS-141` behind it**, on a deck holding a duration or a colour in a custom property, which the check could not see before; **`DS-219`** on a label whose ground is an ancestor rather than its parent, because the check now walks the whole chain ([T-256](../tasks/T-256-ds-219-cannot-see-a-painted-svg-ancestor.md)); and **`DS-106`** on the word its check omitted ([T-229](../tasks/T-229-ds-106s-check-omits-a-word-the-rule-names.md)). **Nothing newly fails for `.turn` or the three optional tokens**: a part at `0+` and a token whose absence is legal take nothing away, and the four narrowings can only turn a red green. **Derived from the contract diffs and the task records rather than from running this gate against a `v0.6.0` deck** — 177 rule rows at both ends, none added, none removed, and no rule's `label`, `check` or `auto` column moved. | `python tools/deck/shell.py sync <deck> --write` settles the three regions and is the only part a command settles. **Delete `--accent-ink` from the deck's theme region; nothing replaces it.** For `DS-010` and `DS-141`, either bring the duration back under 500 ms or declare `--motion-long` with a reason from `loop`, `illustration`, `emphasis`, `request` and put the number in `--long-dur`. `DS-219` and `DS-106` are per-deck edits a command cannot make: raise the contrast on the pair the row names, and reword the line it quotes. |
| `1.0.0` | **One contracted part became required, and one declaration moved.** `.ruler-tip`, the ruler's readout, is `1` `p` inside `.ruler` and ships in the skeleton ([T-307](../tasks/T-307-give-the-chrome-first-and-last-page-controls-and-a-ruler-whose-marks-a-reader-can-aim-at.md), given its drop by [T-314](../tasks/T-314-give-the-ruler-readout-a-drop-and-let-the-dense-ruler-aim-at-every-slide.md)). **The chart-engine declaration moved** from `<meta name="htmldeck-chart-engine">` in `head` to an `htmldeck-chart-engine:` line in the head comment, because `sync` deleted the `meta` ([T-311](../tasks/T-311-keep-a-decks-chart-engine-declaration-through-a-shell-sync.md)). **Optional** additions, which take nothing away: `.term`, `.term-btn` and `.term-bub`, an inline term and its definition bubble ([T-309](../tasks/T-309-contract-an-inline-term-that-opens-a-definition-bubble.md)); `data-qv-at` on a quick-view opener, naming a heading in the source ([T-271](../tasks/T-271-a-quick-view-scoped-to-a-document-section.md)); the `--term-line` token ([T-315](../tasks/T-315-make-the-inline-terms-underline-a-theme-token.md)). `.is-back` and `.chev` go from `0-1` to `0+` (T-307). A `GENERATOR` region carries `<meta name="generator">` at the installed version, and `sync` writes it ([T-318](../tasks/T-318-mark-a-built-deck-in-its-source-a-colophon-and-a-generator-tag.md)). The **shared shell moved in all three regions**. **A contracted part added is at least a minor, and the major is the owner's ruling of 2026-09-13** that the marketplace release is `1.0.0`. | `shell.py check` on **every deck built before this release**: `SKELETON`, `COMPONENTS` and `SCRIPT` all differ. `check.py` — **`DS-229`**, *every authored part is the element, place and count the contract names*, on every such deck, because none carries a `.ruler-tip`. **`DS-122`** on a deck that declared a chart engine in the old `<meta>`, with a reason naming where the line went (T-311). **Measured rather than derived**, unlike `0.7.0`'s row: today's `check.py` against all four decks this repository shipped at `v0.7.0` failed `DS-229` on each and nothing else. Fifteen rule rows in `DESIGN-SYSTEM.md` changed text, and none of them fails any of the four. | `python tools/deck/shell.py sync <deck> --write` settles the three regions, and `.ruler-tip` and the generator tag with them. **Verified on `sort-window` at `v0.7.0`**: `DS-229` fails, then `sync`, then `check.py` exits 0. For a chart engine declared in a `<meta>`, delete the `meta` and put `htmldeck-chart-engine: engine=…; version=…; licence=…; output=svg` on its own line in the head comment. |
