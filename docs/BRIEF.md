...
| 1–2 | T-132, T-133 | the other two of T-130's three `xs` savings, in its ranked order — every task below is paid at the un-inventoried rate until they land |
| 3 | T-139 | `xs`, and it is here rather than at the bottom because **every closure below it edits the table the defect is in** — it settles the form once instead of three more times |
...
| 6 | T-138 | the other repair to `R8`, and `s` — both fixes land before the method is packaged |
| 7 | T-137 | after 5 and 6, which it is blocked on in substance — the one run's evidence is freshest now and decays |
| 8 | T-128 | prepared, ruled and surveyed |
| 9 | T-119 | an audit that may **remove** rules is cheapest before the six tasks that build to them |
| 10 | T-114 | the ruler is the one thing still known to bite past 13 slides |
| 11 | T-109 | the hub of the source cluster; 12–14 are rework if it lands after them |
| 12–14 | T-110, T-118, T-121 | that cluster, downstream of the component decision |
| 15 | T-115 | needs 10 and 11 to have settled what the shell offers |
| 16 | T-117 | independent |
| 17 | T-097 | cheap, and the same class as the closed T-095 — an excusal that went stale when its rule got an instrument |
| 18 | T-054 | after 9, which may change which clauses are worth recording |
