...
- ~~A deck renders correctly with the network disabled.~~ **Met.**
  [`examples/reference-deck.html`](../examples/reference-deck.html) — 12 slides, 307 KB, zero
...
- ~~The build check demonstrated failing on each class of problem it claims to catch.~~ **Met
  ...
  `python tools/deck/check.py <deck> [--sources <dir>] [--print-pages] [--json]` decides **92 of the
  122 rules** the ruleset puts in a gate's jurisdiction and **names the other 30 with a reason and a
