...
| [T-066](../tasks/T-066-make-the-absent-subject-rule-a-fixture-instead-of-a-sweep.md) — make that rule a fixture | **Open.** The same defect has now been found three times, the third after the fix shipped. A hand-run sweep is not a gate. |
| [T-067](../tasks/T-067-the-published-upgrade-instructions-do-not-upgrade.md) — the upgrade instructions do not upgrade | **Open.** Two release notes tell an affected user to run a command that leaves them on the broken version. |
...
- ~~The build check demonstrated failing on each class of problem it claims to catch.~~ **Met
  ...
  113 rules** the ruleset puts in a gate's jurisdiction and **names the other 32 with a reason and a
