...
says what to do next, because sixteen open tasks and **no `blocked_by` edge between any of them**
...
| 1 | T-139 | `xs`, and it is here rather than at the bottom because **every closure below it edits the table the defect is in** — it settles the form once instead of three more times |
| 2 | T-134 | the enabler; it decides what a later cut to `../CLAUDE.md` is allowed to remove |
