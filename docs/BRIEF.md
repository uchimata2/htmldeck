...
> | | |
> | :--- | :--- |
> | [`DESIGN-SYSTEM.md`](DESIGN-SYSTEM.md) | **The operative ruleset.** Each rule carries a stable `DS-nnn` ID, a **hard/default/guidance** label, a **Check** value — `auto` · `render` · `judge` · none — and a **Reach** value, added 2026-08-09 by [T-037](../tasks/T-037-record-in-the-ruleset-which-rules-no-check-can-reach.md): `Check` routes a rule to the build check, the render pass or the evaluator; **`Reach` says whether a check can get at it at all**, which `Check` never did. **The counts are not written here.** `python tools/deck/ruleset.py --counts` prints every one — the table's row total and the declared total, which differ by **DS-000**, the override clause stated as prose in §0 rather than as a row; the split by label, by `Check` and by `Reach`; and why `Reach —` is the `judge` rules *plus* the rules whose `Check` is `—`, not "every `judge` rule". *This row carried ten of those figures beside the instruction to re-derive them, and seven were stale within a fortnight of the instruction being written; they left the row on 2026-09-01 ([T-236](../tasks/T-236-tier-1-and-the-brief-against-what-they-measure.md)).* Loaded on demand; the skill body must not paraphrase it. |
...
| **Delivery mode** | **Embed by default.** Two modes, not three: `portable` (everything inlined, zero external references — the four decks shipped here measured 316 to 427 KB on 2026-08-30, against the 192 KB R5 measured on 2026-08-06, because embedding stayed cheap while the decks grew) is the default and the only shipping mode; `linked` (CDN) exists **for the authoring loop only** and a deck built with it is a defect the critique pass flags. No local-files mode. Settled 2026-08-06 on [R5 §4](research/R5-assets-and-licences.md) — see below. |
...
- ~~A deck renders correctly with the network disabled.~~ **Met.**
  [`examples/reference-deck.html`](../examples/reference-deck.html) — 12 slides, 318 KB, zero
