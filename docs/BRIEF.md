...
| [T-081](../tasks/T-081-the-installed-taskmd-is-two-minor-versions-behind.md) | The installed taskmd is 0.1.1 and upstream ships 0.3.0, so the gates enforce rules two releases old and the project records superseded behaviour as current. Raised by T-073, which predicted an output line that could not appear. |
...
| [T-083](../tasks/T-083-the-generated-example-deck-fails-a-hard-rule-and-nothing-recorded-it.md) | `examples/sort-window/` fails DS-064 at 15.0 px against a 16 px floor, and did before T-071 touched it. The repository's own evidence that the generator works is public and red. |
| [T-084](../tasks/T-084-the-contents-bound-fixture-counts-a-deck-that-no-longer-exists.md) | `contents_bound.py` refuses to start: its fixture counts 12 contents boxes and the reference deck builds 13, since T-069. It blocks T-036, which is the first open-to-open dependency this backlog has had. |
