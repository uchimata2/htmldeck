...
| 1 | T-134 | the enabler; it decides what a later cut to `../CLAUDE.md` is allowed to remove |
| 2 | T-136 | it can invalidate rows below it: a technique the first pass missed may raise a finding that outranks them |
| 3 | T-138 | the other repair to `R8`, and `s` — both fixes land before the method is packaged |
| 4 | T-137 | after 2 and 3, which it is blocked on in substance — the one run's evidence is freshest now and decays |
...
| 19 | T-113 | research, and it decides ground 20–23 build on |
| 20–21 | T-112, T-111 | the motion cluster, in that order |
| 22 | T-041 | named in *what PH1 shipped without* |
| 23 | T-057 | `xl`, and the least certain to be worth it |
