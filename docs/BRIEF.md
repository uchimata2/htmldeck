...
| **Printing** | **Not a requirement.** An optional mode the user can force on to make a deck printable. It must never shape the interaction design. **Measured — [R7](research/R7-printable-mode.md).** The mode is the **paginated stage**, one slide per page; the reading view is not a print target. It costs 3 KB and constrains nothing, so the position holds — but "optional" obliges it to say what it drops, and it drops **38.6% of a deck's text** (everything behind progressive disclosure) plus the reader's paper choice. R7 §5 is the list. |
